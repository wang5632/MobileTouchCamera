...
#pragma warning restore 414
        ...
        public static Action<MobileTouchCamera> PositionOrSizeChangedEvent;
        ...
        public float CamZoom
        {
            ...
            set
            {
                if (Cam.orthographic == true)
                {
                    var positionOrSizeChanged = Math.Abs(Cam.orthographicSize - value) >= Mathf.Epsilon;
                    Cam.orthographicSize = value;
                    if (positionOrSizeChanged)
                    {
                        PositionOrSizeChangedEvent?.Invoke(this);
                    }
                }
                else
                {
                    ...
                    {
                        Vector3 camCenterIntersection = GetIntersectionPoint(GetCamCenterRay());
                        SetTransformPosition(camCenterIntersection - Transform.forward * value);
                    }
                    else
                    {
        ...
        public float CamOverDragMargin => camOverdragMargin;
        ...
        public Vector2 CamPosMin { get; private set; }
        public Vector2 CamPosMax { get; private set; }
        ...
        {
            var positionOrSizeChanged = Transform.position != position;
        ...
        private void UpdatePosition(float deltaTime)
        {
            ...
            if (IsDragging == true || IsPinching == true)
            {
                ...
                if (IsSmoothingEnabled == true)
                {
                    SetTransformPosition(Vector3.Lerp(Transform.position, targetPositionClamped,
                        Mathf.Clamp01(Time.deltaTime * camFollowFactor)));
                }
                else
                {
                    SetTransformPosition(targetPositionClamped);
                }
            ...
            SetTransformPosition(GetClampToBoundaries(camPos));
        }
        ...
        public void LateUpdate()
        {
            ...
            if (Mathf.Approximately(mouseScrollDelta, 0) == false)
            {
                ...
                else
                {
                    ...
                    SetTransformPosition(Transform.position + pinchFocusVector * multiplier);
                }
            }
